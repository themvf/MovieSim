...
import * as N from "./narrative.js?v=0.35.0";
import * as E from "./engine.js?v=0.35.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.35.0";
const BUILD = "0.10.0";
...
function slate() {
 ...
 ${pressTeaser()}${clientBoard()}${commissionBoard()}${peopleBoard()}${opportunityBoard()}<div class="slate-layout"><section><div class="section-title"><h2>Your productions <span>${s.movies.length}</span></h2><div class="segmented">${[
   ["active", "Active"],
   ["streaming", "Streaming"],
   ["catalog", "Catalog"],
   ["all", "All"],
...
function movieDetail(m) {
 ...
 ${clientFilm(m)}${commissionFilm(m)}${delayStatus(m)}${chemistryFilm(m)}
...
function pressPage(){
 ...
 return `<section class="press-paper"><header class="press-masthead"><div>THE BUSINESS OF MAKING BELIEVE</div><h2>The Final Cut<span>●</span></h2><p>YOUR INDUSTRY JOURNAL <span>${E.date(s.week).label} · WEEK ${E.date(s.week).week}</span></p></header>${articles.length?`<div class="press-grid">${articles.slice(0,6).map(article).join('')}</div>${articles.length>6?`<details class="press-archive"><summary>Earlier headlines · ${articles.length-6}</summary><div class="press-grid">${articles.slice(6).map(a=>article(a,1)).join('')}</div></details>`:''}`:'<div class="press-empty"><span>✦</span><h3>The next headline is yours.</h3><p>Release your first film. We’ll cover the milestones.</p></div>'}<footer>Recorded when discovered. Box office means ticket sales; profit uses studio receipts.</footer></section>`;
}
