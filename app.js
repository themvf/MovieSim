...
import {GENRE_SYMBOLS} from "./card-genres.js?v=0.36.0";
import * as CL from "./clients.js?v=0.36.0";
import * as CH from "./chemistry.js?v=0.36.0";
import * as SF from "./scifi.js?v=0.36.0";
import * as N from "./narrative.js?v=0.36.0";
import * as E from "./engine.js?v=0.36.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.36.0";
const BUILD = "0.10.0";
...
function handle(e) {
  ...
  switch (a) {
    ...
    case "scifiEdit": {const film=E.movie(s,id);open("scifi",{id,genre:film.genre,cards:film.scifiCards?structuredClone(film.scifiCards):SF.blank(),title:film.title,scale:film.scale});break;}
    case "sfPick": {
...
dialog.addEventListener("submit", (e) => {
  ...
  if(f.id === "sf-form") {
    ...
    else transact("original",{title:d.title,genre:view.genre??"Sci-fi",subgenre:E.GENRES[view.genre??"Sci-fi"][0],scale:d.scale,scifiCards:view.cards,pitch:view.pitch},m=>{view={kind:"movie",id:m.id};tab="slate";});
  }
...
function scifiRoom(){
 const genre=view.genre??'Sci-fi',d=view.cards,valid=SF.validate(d,genre),editing=!!view.id,cost=editing?50:100*(E.SCALES.indexOf(view.scale)+1)*LIFE.scriptMultiplier(s,genre);
 const changed=!editing||JSON.stringify(d)!==JSON.stringify(E.movie(s,view.id).scifiCards);
 return modal('Build your '+genre.toLowerCase()+' movie',`<div class="sf-builder">${view.pitch?`<div class="notice-banner">Passion project · Keep ${s.life.pitches.find(x=>x.id===view.pitch).protect.map(([,c])=>h(c)).join(" + ")} and hire ${h(E.person(s,s.life.pitches.find(x=>x.id===view.pitch).person).name)}. Their quoted fee is 25% lower.</div>`:""}<p class="sf-intro">Choose 1–2 cards in each section. Your first selection is primary; your second supports it. Tap a selected card to remove it.</p><form id="sf-form"><div class="form-grid"><label>Movie title<input name="title" required maxlength="60" value="${h(view.title)}" ${editing?'readonly':''} placeholder="Name your movie"></label><label>Production scope<select name="scale" ${editing?'disabled':''}>${E.SCALES.map(x=>`<option value="${x}" ${view.scale===x?'selected':''}>${E.scopeName(x)}</option>`).join('')}</select></label></div></form>${SF.sections(genre).map((section,row)=>`<section class="sf-section"><header><div><span class="eyebrow">${section.name}</span><h3>${section.question}</h3></div><span>${d[section.id].length} / 2</span></header><div class="sf-cards">${section.cards.map((card,col)=>{const rank=d[section.id].indexOf(card);return button(`${cardArt(genre,row,col)}<strong>${h(card)}</strong>${rank>=0?`<span class="sf-rank">${rank+1}</span>`:''}`,'sfPick',`data-section="${section.id}" data-card="${card}" aria-pressed="${rank>=0}" aria-label="${h(card)}${rank>=0?rank===0?', primary':', supporting':''}"`,'sf-card');}).join('')}</div></section>`).join('')}<section class="sf-focus"><span class="eyebrow">CREATIVE DEMANDS</span><p>${SF.selected(d,genre).length?SF.demands(d,genre).filter(([,n])=>n>0).slice(0,2).map(([k])=>h(SF.DIMENSIONS[k])).join(' and '):'Select ingredients to reveal your creative focus.'}</p><small>Cast, direction, writing and production determine execution. Additional cards are not automatic quality bonuses.</small></section><footer class="sf-footer"><div><strong>${E.accountMoney(cost)} ${editing?'rewrite':'screenplay'}</strong><small>${editing?'Locks when filming begins.':'Three weeks of development. Cast and production paid separately.'}</small></div><button type="submit" form="sf-form" class="primary" ${!valid||!changed?'disabled':''}>${editing?'Approve revised cards':'Commission screenplay →'}</button></footer></div>`,genre.toUpperCase()+' CARD LAB');
}
