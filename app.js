import * as N from "./narrative.js?v=0.28.0";
import * as E from "./engine.js?v=0.28.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.28.0";
const BUILD = "0.10.0";
...
function productionPreview(m) {
  ...
  if(m.narrativePack){let note=document.getElementById("story-budget-preview");if(!note){note=document.createElement("p");note.id="story-budget-preview";note.className="story-budget-note";f.append(note);}note.textContent=`Story execution: ${N.executionPenalty(m.narrative,b).toFixed(1)} quality points at risk with this effects allocation.`;}
...
function narrativeRoom(m){
 ...
 const scene=step<0?narrativeCast(m,d,true):`<div class="scene-heading"><span class="scene-number" aria-hidden="true">${beatIcons[step]}</span><div><span class="eyebrow">${step+1} / 6 · ${step<1?'ACT I':step<3?'ACT II':'ACT III'}</span><h3>${N.STEPS[step]}</h3><p>${['What starts this movie?','What breaks the original plan?','What does Alex choose to do?','How is the conflict resolved?','What does the outcome take away?','What remains of Voss’s threat?'][step]}</p></div></div><div class="story-options">${shown.map(c=>button(`<span class="choice-mark" aria-hidden="true">${c.lock?'🔒':d.beats[step]===c.id?'✓':'○'}</span><strong>${h(c.title)}</strong><small>${h(c.text)}</small><span class="story-tag">${c.lock?'Needs '+h(c.lock):c.effects?'Effects +'+E.accountMoney(c.effects):'Character / plot'}</span>`,'narrativePick',`data-choice="${c.id}" ${c.lock?'disabled':''} aria-pressed="${d.beats[step]===c.id}"`,'story-option')).join('')}</div>${options.filter(c=>c.lock).length>shown.filter(c=>c.lock).length?'<p class="small muted">More possibilities appear as earlier story facts change.</p>':''}`;
 return modal('Build your movie',`<div class="narrative-room ${step>=0?'editing-step':''}"><header class="story-banner"><span class="eyebrow">THE SIGNAL ASH SPEC · SUPERHERO THRILLER</span><h3>One city. Four lives.<br>Your version.</h3><p>The purchased draft is complete. Film it as written, or change a part. Approving a changed draft costs $50,000.</p></header>${narrativeTimeline(d,step,true)}${button('♙ Characters','narrativeStep','data-step="-1"','outline')}<section class="story-workbench">${scene}</section>${e.errors.length?`<div class="story-repair" role="status"><strong>${e.errors.length} connection(s) need revision</strong><p>Your other selections stay in place. Resolve these before approval.</p>${e.errors.map(x=>`<p>${x.step>=0?N.STEPS[x.step]+': ':''}${h(x.reason)}</p>`).join('')}</div>`:''}<div class="story-draft-summary"><span>${e.pressure} effects ambition</span><strong>${E.accountMoney(e.effects)} effects target</strong><small>Underfunding risks up to 8 quality points. Four contracted actors required; story deaths do not affect actors.</small></div><div class="story-navigation">${button(step<=0?'Characters':'← '+N.STEPS[step-1],'narrativeStep',`data-step="${Math.max(-1,step-1)}"`,'outline')}${step<5?button('Next: '+N.STEPS[step+1]+' →','narrativeStep',`data-step="${step+1}"`,'primary'):''}</div>${button(changed?'Approve this rewrite · $50,000':'Draft approved · ready to cast','saveNarrative',`data-id="${m.id}" ${!changed||!e.valid?'disabled':''}`,'primary full')}${button('Return to film packaging','movie',`data-id="${m.id}"`,'outline full')}${button('Restore purchased draft · free','restoreStory',`data-id="${m.id}" ${JSON.stringify(m.narrative)===JSON.stringify(m.originalNarrative)?'disabled':''}`,'text-button full')}<p class="small muted">Unapproved changes are discarded when you leave. All story edits lock at greenlight.</p></div>`,'THE WRITERS’ ROOM');
}
function narrativeReport(m){
 if(!Number.isFinite(m.fans)||!Number.isFinite(m.critics))return '';
 const e=N.evaluate(m.narrative),links=e.links.slice(-3),loss=N.executionPenalty(m.narrative,m.budget);
 return `<section class="panel story-report narrative-report"><span class="eyebrow">STORY REPORT CARD · ${m.narrativeRevisions??0} REVISIONS</span><h3>${h(m.title)}: the choices behind the film</h3>${narrativeTimeline(m.narrative)}<div class="payoff-grid"><article><span class="report-symbol">↗</span><h4>Setup → payoff</h4>${links.length?links.map(l=>`<div class="payoff-link"><strong>${h(e.selected[l.from].title)}</strong><span aria-hidden="true">↓</span><strong>${h(e.selected[l.to].title)}</strong><small>Connected by ${h(N.LABELS[l.tag]??l.tag)}</small></div>`).join(''):'<p>The finale relies on character facts rather than a planted scene. Add a stronger earlier setup.</p>'}</article><article><span class="report-symbol">◈</span><h4>Connection to develop</h4><p>${h(e.weakness)}</p></article><article><span class="report-symbol">ϟ</span><h4>Production consequence</h4><p>${E.accountMoney(e.effects)} effects target; ${E.accountMoney(m.budget?.effects??0)} allocated.</p><strong>${loss?loss.toFixed(1)+' quality points lost to story ambition':'Story effects target met'}</strong><p>Other production and casting factors still affect the finished film.</p></article></div><div class="scores">${stat('AUDIENCE',E.score(m.fans)+'/100')}${stat('CRITICS',E.score(m.critics)+'/100')}${stat('FILM QUALITY',E.score(m.quality)+'/100')}</div><p class="small muted">Structural observations explain selected story facts. These whole-film scores do not establish how viewers felt about any individual scene.</p><details><summary>The characters on screen</summary>${narrativeCast(m,m.narrative)}</details></section>`;
}
