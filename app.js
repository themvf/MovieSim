import * as COMM from './commissions.js?v=0.40.0';
import {nextHire, standardOffer, deadlineWindows, commissionEstimate} from './experience.js?v=0.40.0';
import * as REC from "./reception.js?v=0.40.0";
import * as LIFE from "./studio-life.js?v=0.40.0";
import {GENRE_SYMBOLS} from "./card-genres.js?v=0.40.0";
import * as CL from "./clients.js?v=0.40.0";
import * as CH from "./chemistry.js?v=0.40.0";
import * as SF from "./scifi.js?v=0.40.0";
import * as N from "./narrative.js?v=0.40.0";
import * as E from "./engine.js?v=0.40.0";
import { portrait, poster, studioArt, settingCardArt, escapeHtml as h } from "./art.js?v=0.40.0";
const BUILD = "0.10.0";
...
function movieDetail(m) {
 ...
 ${m.scifiCards?`<section class="sf-concept"><span class="eyebrow">YOUR MOVIE CARDS</span><p>${h([...SF.selected(m.scifiCards,m.genre).map(c=>c.name),...(m.endingCards??[])].join(" · "))}</p><p>Creative focus: ${SF.demands(m.scifiCards,m.genre).slice(0,2).map(([k])=>h(SF.DIMENSIONS[k])).join(" and ")}.</p></section>`:""}
 ${SF.genres.includes(m.genre)&&m.parent&&["development","packaging"].includes(m.stage)?`<section class="people-panel"><h3>Develop the sequel’s direction</h3><p>${m.scifiCards?'The original cards are your starting draft. Keep them unchanged for free, or revise any section.':'Build a new card concept for this sequel.'} Approved changes cost $50,000 and lock when filming starts.</p>${button('Change sequel cards','scifiEdit',`data-id="${m.id}"`,'primary full')}</section>`:m.stage==="packaging"&&m.scifiCards?button("Revise cards","scifiEdit",`data-id="${m.id}"`,'outline full'):''}
...
function handle(e) {
  ...
  switch (a) {
    ...
    case "lifePitch":{const x=s.life.pitches.find(x=>x.id===b.dataset.pitch);open('scifi',{genre:x.genre,cards:structuredClone(x.cards),pitch:x.id,title:'',scale:'Small'});break;}
    ...
    case "buy":
      ...
      break;
    case "scifiNew": if(view?.kind==="opening"){s.notices=s.notices.filter(n=>!(n.kind==="opening"&&n.id===view.id));save();}open("scifi",{genre:b.dataset.genre??"Sci-fi",cards:SF.blank(),title:"",scale:"Small"});break;
    case "scifiEdit": {const film=E.movie(s,id);open("scifi",{id,endingCards:structuredClone(film.endingCards??[]),publicPromise:film.publicPromise??"As written",genre:film.genre,cards:film.scifiCards?structuredClone(film.scifiCards):SF.blank(),title:film.title,scale:film.scale});break;}
    case "albumTab": view.albumTab=b.dataset.section;drawDialog();{const n=dialog.querySelector(".album-tabs"),a=n?.querySelector("[aria-pressed=true]");if(a)n.scrollLeft=a.offsetLeft-n.offsetLeft-20;}break;
    case "endingPick": {const top=dialog.querySelector(".modal-body").scrollTop;view.endingCards??=[];const card=b.dataset.card,i=view.endingCards.indexOf(card);if(i>=0)view.endingCards.splice(i,1);else if(view.endingCards.length<2){const next=[...view.endingCards,card];if(!REC.valid(next)){toast('Those outcomes contradict each other. Remove one first.');break;}view.endingCards=next;}else{toast('Choose at most two endings. Remove one first.');break;}drawDialog();dialog.querySelector('.modal-body').scrollTop=top;[...dialog.querySelectorAll('[data-action="endingPick"]')].find(el=>el.dataset.card===card)?.focus({preventScroll:true});break;}
    case "endingInfo": toast(REC.ENDINGS.find(c=>c.name===b.dataset.card)?.description??'');break;
    case "sfPick": {
...
dialog.addEventListener("submit", (e) => {
  ...
  const f = e.target,
    ...
    v = { ...view };
  if(f.id === "sf-form") {
    if(view.id) transact("scifiRewrite",{id:view.id,cards:view.cards,endingCards:view.endingCards,publicPromise:view.publicPromise??"As written"},()=>{view={kind:"movie",id:v.id};});
    else transact("original",{title:d.title,genre:view.genre??"Sci-fi",subgenre:E.GENRES[view.genre??"Sci-fi"][0],scale:d.scale,scifiCards:view.cards,endingCards:view.endingCards,publicPromise:view.publicPromise??"As written",pitch:view.pitch},m=>{view={kind:"movie",id:m.id};tab="slate";});
  }
...
dialog.addEventListener("change", async (e) => {
  if(e.target.id === "genre" && SF.genres.includes(e.target.value)){const f=e.target.form;open("scifi",{genre:e.target.value,cards:SF.blank(),title:f.elements.title.value,scale:f.elements.scale.value});return;}
...
function scifiRoom(){
 const genre=view.genre??'Sci-fi',d=view.cards,editing=!!view.id,endings=view.endingCards??[],section=view.albumTab??'setting';
 const cost=editing?50:100*(E.SCALES.indexOf(view.scale)+1)*LIFE.scriptMultiplier(s,genre),sections=[...SF.sections(genre),{id:'ending',name:'Ending',question:'Choose your ending',cards:REC.ENDINGS.map(c=>c.name)}],active=sections.find(x=>x.id===section)??sections[0];
 const valid=SF.validate(d,genre)&&REC.valid(endings),count=SF.selected(d,genre).length+endings.length;
 return modal('Cinema Collection',`<div class="cinema-album"><form id="sf-form"><label>Movie title<input name="title" required maxlength="60" value="${h(view.title)}" ${editing?'readonly':''} placeholder="Untitled movie"></label><details class="album-settings"><summary>Production & campaign</summary><div class="form-grid"><label>Scope<select name="scale" ${editing?'disabled':''}>${E.SCALES.map(x=>`<option ${view.scale===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Campaign promise<select name="publicPromise">${REC.PROMISES.map(x=>`<option ${(view.publicPromise??'As written')===x?'selected':''}>${x}</option>`).join('')}</select></label></div><p class="small muted">Campaign promise sets the expected ending tone. A mismatch can disappoint viewers. As written makes no extra promise.</p></details></form>${view.pitch?'<p class="small">Passion project: retain the promised ingredients and hire the pitching talent.</p>':''}<p class="album-step">Step ${sections.indexOf(active)+1} of ${sections.length} · ${sections.filter(x=>(x.id==='ending'?endings:d[x.id]).length).length} complete</p><nav class="album-tabs" aria-label="Card category">${sections.map(x=>button(x.name,'albumTab',`data-section="${x.id}" aria-pressed="${active.id===x.id}"`,'outline')).join('')}</nav><h3>${h(active.question)}</h3><p class="muted small">Pick 1–2 cards. Tap again to remove.</p>${cardGuidance(d,genre,endings)}<div class="album-grid">${active.cards.map((name,col)=>{const isEnd=active.id==='ending',selected=(isEnd?endings:d[active.id]).includes(name),c=REC.ENDINGS.find(x=>x.name===name);return `<article>${button(`${isEnd?`<span class="ending-art" aria-hidden="true">${c.symbol}</span>`:cardArt(genre,sections.indexOf(active),col)}<strong>${h(name)}</strong>${selected?'<span class="album-check">✓</span>':''}`,isEnd?'endingPick':'sfPick',`data-section="${active.id}" data-card="${h(name)}" aria-pressed="${selected}"`,'sf-card album-card')}${isEnd?button('i','endingInfo',`data-card="${h(name)}" aria-label="About ${h(name)}"`,'album-info'):''}</article>`;}).join('')}</div>${endings.length?`<p class="album-interpretation">${h(REC.interpretation(endings,d))}</p>`:''}<details><summary>View movie · ${count} cards</summary>${sections.map(x=>`<p><strong>${x.name}:</strong> ${h((x.id==='ending'?endings:d[x.id]).join(' · ')||'Choose cards')}</p>`).join('')}<p class="small">${endings.length?'Creative focus: '+Object.entries(REC.demand(d,SF.sections(genre),endings)).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([k])=>SF.DIMENSIONS[k]).join(' · '):'Choose an ending to reveal the creative focus.'} Production determines execution. More cards do not guarantee better reviews.</p></details><footer class="album-footer"><span>${count} cards · ${E.accountMoney(cost)}</span>${valid?`<button type="submit" form="sf-form" class="primary">${editing?'Approve rewrite':'Commission screenplay →'}</button>`:!(active.id==='ending'?endings:d[active.id]).length?'<button class="primary" disabled>Choose at least 1 card</button>':button('Next: '+(sections.find(x=>!(x.id==='ending'?endings:d[x.id]).length)?.name??'Ending')+' →','albumTab',`data-section="${sections.find(x=>!(x.id==='ending'?endings:d[x.id]).length)?.id??'ending'}"`,'primary')}</footer></div>`,genre.toUpperCase());
}
...
function chemistryCandidate(m,p,role){
 ...
 return `<div class="chemistry-note"><strong>${h(CH.style(p))}</strong><small>Pair chemistry</small>${pairs.map(pair=>`<div class="chemistry-compact"><span>${h(E.person(s,pair.ids[1]).name)}</span><b class="${pair.effect>0?'mint':pair.effect<0?'negative':'muted'}">${pair.effect>0?'↗':pair.effect<0?'↘':'—'} ${chemistryEffect(pair.effect)}</b></div>`).join('')}${pairs.length?'':'<small>Compare after attaching a co-star.</small>'}</div>`;
}
function chemistryFilm(m){
 ...
 return `<section class="people-panel chemistry-panel"><span class="eyebrow">CAST CHEMISTRY</span><h3>${snapshot?'The people between the scenes':'Who brings out their best?'}</h3>${pairs.length?pairs.map(pair=>`<div class="chemistry-pair"><strong>${pair.ids.map(id=>h(E.person(s,id).name)).join(' & ')}</strong><p>${h(pair.label)} · ${chemistryEffect(pair.effect)} chemistry · ${pair.films} films</p>${pair.outcome?`<p>${h(pair.outcome)}</p>`:''}</div>`).join(''):'<p>Attach at least two actors to discover how their working styles fit.</p>'}<details><summary>How chemistry works</summary><p class="small muted">${snapshot?'Chemistry locked when filming started.':'Locks at filming.'} Co-star contributions average up to ±4 performance points. At wrap, pairs averaging 60+ build trust; others build tension.</p></details></section>`;
}
...
function fanSentiment(m){
 const f=m?.fanCommunity;if(!f?.ready)return modal('Fan sentiment','<p>No new reactions yet.</p>');
 if(!f.presented){f.presented=true;save();}
 const lead=E.person(s,m.peopleStory?.actors[0]?.id);
 return modal('The audience has a voice',`<div class="fan-popup-hero">${poster(m,true)}<div><span class="eyebrow">${h(m.title)}</span><h3>${h(f.title)}</h3><strong class="fan-score">♥ ${Math.round(m.fans)}<small>/100</small></strong></div></div><blockquote class="fan-quote">${h(f.text)}</blockquote>${f.choice?`<p class="notice-banner">${h(f.response)}</p>`:`<div class="fan-actions">${button('<strong>Spotlight</strong><small>$25,000 · +8% demand for 4 weeks</small>','fanResponse',`data-id="${m.id}" data-choice="spotlight" ${m.stage!=='theaters'?'disabled':''}`,'outline')}${button(`<strong>Credit the cast</strong><small>Free · +2 trust with ${h(lead?.name??'the lead')}</small>`,'fanResponse',`data-id="${m.id}" data-choice="thanks"`,'outline')}${button('<strong>Just listen</strong><small>Free · no change</small>','fanResponse',`data-id="${m.id}" data-choice="listen"`,'outline')}</div><small class="muted">Spotlight applies during the theatrical run; reviews stay unchanged.</small>`}${button(f.choice?'Back to movie':'Decide later','movie',`data-id="${m.id}"`,'text-button full')}`,'FAN SENTIMENT');
}
...
function clientDialog(id){
 ...
 return modal(c.title,`<div class="client-intro"><span class="client-avatar client-${c.id}">${c.initials}</span><div><h3>${h(c.name)}</h3><small>${h(c.role)} · ${h(c.company)}</small></div></div><blockquote class="client-quote">“${h(r.last)}”</blockquote><div class="client-metrics">${stat('ON DELIVERY',E.accountMoney(payment))}${stat(d?'DEADLINE':'RELEASE WITHIN',d?`${Math.max(0,d.due-s.week)} weeks`:`${c.weeks} weeks`)}</div><div class="client-requirements"><span>Sci-fi</span>${c.cards.map(([section,card])=>`<span>${h(card)} ${film?(film.scifiCards?.[section]?.includes(card)?'✓':'— missing'):''}</span>`).join('')}<span>${c.score?`${c.score==='critics'?'Critics':'Fans'} ≥ ${c.minimum}`:'No score target'}</span></div><p class="small">${h(c.tier)} · +${c.premium}% fee. ${h(c.reason)}</p>${briefEstimate(c.weeks,c,d)}${d?`<p class="small muted">Release by ${E.date(d.due).label}, W${E.date(d.due).week}.</p>${film?`<div class="people-row"><strong>${h(film.title)}</strong>${button('Open film','movie',`data-id="${film.id}"`,'outline')}</div>`:`<h3>Attach a film</h3>${s.movies.filter(m=>CL.eligible(s,m)&&CL.matches(m,c)).map(m=>button(h(m.title)+' →','clientBind',`${args} data-id="${m.id}"`,'outline full')).join('')||'<p class="small muted">No uncommitted films match both cards yet.</p>'}${button('Build with cards','scifiNew','','primary full')}<p class="small muted">Choose before filming. Attachment is final.</p>`}`:CL.available(s,id)?`${button('Accept brief','clientAccept',args,'primary full')}${button('Pass for now','clientDecline',args,'text-button full')}`:`<p>${!CL.unlocked(s,id)?'Deliver any commission successfully to unlock rush orders.':r.strikes>=2?'This client is no longer commissioning your studio.':s.week+c.weeks>=E.END?'Not enough time remains for a new brief.':`Next conversation in ${Math.max(0,r.nextOffer-s.week)} weeks.`}</p>`}<p class="client-stakes">Payment requires an on-time release${c.score?` and the ${c.score==='critics'?'critic':'fan'} target`:''}. No extensions. Miss: no payment, 40-week pause. Two misses end requests.</p><details><summary>Your relationship · ${r.wins} delivered · ${r.strikes} missed</summary><p class="small">Deliveries increase future fees, up to three increases. Pass without penalty. Cards may be primary or supporting. One client per film, including Mars Travel. Payment adds to studio receipts; ticket sales and rights are unchanged.</p>${r.history.slice(-4).reverse().map(x=>`<p><strong>${h(x.title)} · ${h(x.outcome)}</strong><small class="block">${h(x.reason)} ${x.payment?E.accountMoney(x.payment):''}</small></p>`).join('')}</details>`,'AN INDUSTRY BRIEF');
}
...
function briefEstimate(weeks,client=null,deal=null){
 const eligible=s.movies.filter(m=>client?CL.eligible(s,m)&&CL.matches(m,client):E.commissionEligible(m)&&m.scifiCards?.setting?.includes('Colony'));
 ...
 return `<section class="brief-estimate"><strong>Release estimate · ${e.total} weeks</strong><details ${view.estimateExpanded?'open':''}><summary>Adjust planning assumptions</summary>${!deal?.movie?`<label>Plan<select id="brief-film"><option value="">New screenplay</option>${eligible.map(m=>`<option value="${m.id}" ${movie?.id===m.id?'selected':''}>${h(m.title)}</option>`).join('')}</select></label>`:''}${!movie?`<label>Scope<select id="brief-scale">${E.SCALES.map(x=>`<option ${x===scale?'selected':''}>${x}</option>`).join('')}</select></label>`:''}<label>Shoot<select id="brief-duration">${Array.from({length:17},(_,i)=>i+4).map(w=>`<option value="${w}" ${w===duration?'selected':''}>${w} weeks</option>`).join('')}</select></label></details><p class="small">${e.development} development + ${e.shoot} filming + 4 setback allowance + 1 release.</p><strong class="${e.buffer<0?'negative':e.buffer<3?'peach':'mint'}">${e.buffer<0?`${-e.buffer} weeks beyond deadline`:e.buffer===0?'No buffer':`${e.buffer} weeks of buffer`}</strong><small class="block">Estimate only; hiring and booking conflicts can add time. ${deal?'The accepted deadline stays fixed.':'The clock starts on acceptance.'} Planning here does not change or attach a film.</small></section>`;
}
