...
import * as N from "./narrative.js?v=0.33.0";
import * as E from "./engine.js?v=0.33.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.33.0";
const BUILD = "0.10.0";
...
function slate() {
 ...
 ${commissionBoard()}${peopleBoard()}${opportunityBoard()}<div class="slate-layout"><section><div class="section-title"><h2>Your productions <span>${s.movies.length}</span></h2><div class="segmented">${[
   ["active", "Active"],
   ["streaming", "Streaming"],
   ["catalog", "Catalog"],
   ["all", "All"],
...
function scripts() {
  return `<section class="sf-invite"><span class="eyebrow">NEW · SCI-FI CARD LAB</span><h2>Your ingredients. Your movie.</h2><p>Mix 30 illustrated cards into a science-fiction concept, then cast it and discover who connects with it.</p>${button("Build a sci-fi movie →","scifiNew","","primary")}</section>${s.market.some(m=>m.narrativePack)?`<section class="story-collection"><div class="section-title"><h2>Authored screenplay collection</h2><span class="muted small">Earlier story experiments</span></div><div class="story-library">${s.market.filter(m=>m.narrativePack).map(m=>`<article class="story-feature" data-theme="${N.packFor(m.narrative).theme}">${poster(m)}<div><span class="eyebrow">${h(m.genre)}</span><h3>${h(m.title)}</h3><p>${h(N.packFor(m.narrative).headline)}</p>${button('Explore the spec →','script',`data-script="${m.id}"`,'primary')}</div></article>`).join('')}</div></section>`:''}<details class="poster-preview"><summary>See the four new movie posters</summary><div class="poster-preview-grid">${E.POSTER_FILMS.map(m=>`<figure>${poster(m,true)}<figcaption>${h(m.title)}</figcaption></figure>`).join("")}</div></details><div class="section-title"><h2>Available screenplays <span>${s.market.length}</span></h2><span class="muted small">New scripts every 13 weeks</span></div><div class="script-grid">${s.market.map((m) => `<article class="script-card"><div class="script-top">${poster(m)}<div>${m.narrativePack?pill("BUILD THIS MOVIE", "gold-pill"):pill(E.scopeName(m.scale))}<span class="eyebrow">${h(m.genre)} / ${h(m.subgenre)}</span><h3>${h(m.title)}</h3>${m.difficulty>=80 && m.quality>=75 ? '<span class="fresh-note">Prestige project · some superstars may lower their fee</span>' : ""}<p>${h(m.premise)}</p></div></div><div class="script-stats">${stat("QUALITY ESTIMATE", ratingRange(E.range(m.quality, s.departments.Development)))}${stat("ROLE DIFFICULTY", `${E.score(m.difficulty)}/100`)}${stat("CAST", `${m.roles.length} roles`)}</div><div class="card-bottom"><strong>${E.accountMoney(m.price)} <small>incl. sequel rights</small></strong>${button("Read & acquire →", "script", `data-script="${m.id}"`, "outline")}</div></article>`).join("")}</div><p class="muted small">Quality is an estimate. There is no prescribed production budget—your choices and your results will teach you the economics.</p>`;
}
...
function genreStrengths(p) {
  ...
  return `${p.personality?`<p class="personality-tag">${p.kind==="director"?h(E.workingStyle(p).name):`Wants to try ${h(p.personality.goal)}`}</p>`:""}<div class="genre-strengths"><span><b>Strong in</b> ${sorted
    .slice(0, 2)
    .map(([g]) => g)
    .join(
      " · ",
...
function movieDetail(m) {
 ...
 ${m.scifiCards?`<section class="sf-concept"><span class="eyebrow">YOUR SCI-FI CONCEPT</span><p>${h(SF.selected(m.scifiCards).map(c=>c.name).join(" · "))}</p><p>Creative focus: ${SF.demands(m.scifiCards).slice(0,2).map(([k])=>h(SF.DIMENSIONS[k])).join(" and ")}.</p></section>`:""}
 ${m.genre==="Sci-fi"&&m.parent&&["development","packaging"].includes(m.stage)?`<section class="people-panel"><h3>Develop the sequel’s direction</h3><p>${m.scifiCards?'The original cards are your starting draft. Keep them unchanged for free, or revise any section.':'Build a new sci-fi concept for this sequel.'} Approved changes cost $50,000 and lock when filming starts.</p>${button('Change sequel cards','scifiEdit',`data-id="${m.id}"`,'primary full')}</section>`:m.stage==="packaging"?button(m.scifiCards?"Revise your cards":"Enter the writers’ room",m.scifiCards?"scifiEdit":"writersRoom",`data-id="${m.id}"`,'outline full'):''}
 ${commissionFilm(m)}${delayStatus(m)}${chemistryFilm(m)}
...
function drawDialog() {
  ...
  if(v.kind==="productionDelay")return modal("A word from your director",delayPanel(m),"PRODUCTION DELAY");
  if (v.kind === "movie") {
  ...
  if (v.kind === "script") {
    ...
    return modal(
      h(sc.title),
      `<div class="detail-hero">${poster(sc, true)}<div>${pill(E.scopeName(sc.scale))}<h3>${h(sc.genre)} / ${h(sc.subgenre)}</h3><p>${h(sc.premise)}</p><p>Script quality estimate: <strong>${ratingRange(E.range(sc.quality, s.departments.Development))}</strong></p><p>Role difficulty: <strong>${E.score(sc.difficulty)}/100</strong></p><p>Cast: ${sc.roles.join(", ")}</p></div></div><p class="scope-description">${E.SCOPES[sc.scale].description}</p><p>Includes the screenplay and sequel rights. You can rename the movie after acquisition. Production, talent, and marketing are separate expenses.</p>${sc.narrativePack?`<section class="story-budget-note"><strong>A complete, playable draft</strong><p>Optional rewrites: six linked scenes, four character presets and a causal report after release. Four actors must be hired. The original draft targets ${E.accountMoney(N.evaluate(sc.narrative).effects)} of base ${h(N.packFor(sc.narrative).departmentLabel.toLowerCase())} allocation; more demanding scenes can need more.</p></section>${narrativeTimeline(sc.narrative)}<details><summary>Meet the fictional characters</summary>${narrativeCast(sc,sc.narrative)}</details>`:""}${button(`Acquire screenplay · ${E.accountMoney(sc.price)}`, "buy", `data-script="${sc.id}"`, "primary full")}`,
      "SCRIPT ACQUISITION",
    );
  }
  if (v.kind === "scifi") return scifiRoom();
  if (v.kind === "original")
    return modal(
      "A story of your own",
  ...
  if (v.kind === "writersRoom") return writersRoom(m);
...
function handle(e) {
  ...
  switch (a) {
    ...
    case "writersRoom":open("writersRoom",{id,draft:[...E.storyOutline(E.movie(s,id))],narrative:structuredClone(E.movie(s,id).narrative),step:-1});break;
    case "narrativeStep":view.step=Number(b.dataset.step);drawDialog();const workbench=dialog.querySelector(".story-workbench");if(workbench){workbench.tabIndex=-1;workbench.focus({preventScroll:true});workbench.scrollIntoView({block:"start"});}break;
    case "narrativeCast":{const role=Number(b.dataset.role),choice=b.dataset.choice,scroll=dialog.scrollTop;view.narrative.characters[role]=choice;drawDialog();dialog.querySelectorAll(".story-person details")[role].open=true;dialog.scrollTop=scroll;dialog.querySelector(`[data-action="narrativeCast"][data-role="${role}"][data-choice="${choice}"]`)?.focus({preventScroll:true});break;}
    case "narrativePick":{const choice=b.dataset.choice,scroll=dialog.scrollTop;view.narrative.beats[view.step]=choice;drawDialog();dialog.scrollTop=scroll;dialog.querySelector(`[data-action="narrativePick"][data-choice="${choice}"]`)?.focus({preventScroll:true});break;}
    case "saveNarrative":transact("storyRewrite",{id,draft:view.narrative},()=>{view={kind:"movie",id};});break;
    case "storyPick":view.draft[Number(b.dataset.act)]=b.dataset.choice;drawDialog();break;
    case "saveStory":transact('storyRewrite',{id,acts:view.draft},()=>{view={kind:'movie',id};});break;
    case "restoreStory":transact('storyRewrite',{id,restore:true},()=>{view={kind:'movie',id};});break;
    ...
    case "buy":
      ...
      break;
    case "scifiNew": open("scifi",{cards:SF.blank(),title:"",scale:"Small"});break;
    case "scifiEdit": {const film=E.movie(s,id);open("scifi",{id,cards:film.scifiCards?structuredClone(film.scifiCards):SF.blank(),title:film.title,scale:film.scale});break;}
    case "sfPick": {
      const section=b.dataset.section,card=b.dataset.card,chosen=view.cards[section],index=chosen.indexOf(card);
      if(index>=0)chosen.splice(index,1);else if(chosen.length<2)chosen.push(card);else {toast("Choose at most two. Remove a selection first.");break;}
      const top=dialog.querySelector(".modal-body").scrollTop;drawDialog();dialog.querySelector(".modal-body").scrollTop=top;
      dialog.querySelector(`[data-section="${section}"][data-card="${card}"]`)?.focus({preventScroll:true});break;
    }
    case "original":
      open("original");
      break;
...
dialog.addEventListener("submit", (e) => {
  ...
  const f = e.target,
    ...
    v = { ...view };
  if(f.id === "sf-form") {
    if(view.id) transact("scifiRewrite",{id:view.id,cards:view.cards},()=>{view={kind:"movie",id:v.id};});
    else transact("original",{title:d.title,genre:"Sci-fi",subgenre:E.GENRES["Sci-fi"][0],scale:d.scale,scifiCards:view.cards},m=>{view={kind:"movie",id:m.id};tab="slate";});
  }
...
dialog.addEventListener("input", (e) => {
  if(e.target.closest("#sf-form")){if(e.target.name==="title") view.title=e.target.value;if(e.target.name==="scale"){view.scale=e.target.value;const top=dialog.querySelector(".modal-body").scrollTop;drawDialog();dialog.querySelector(".modal-body").scrollTop=top;}}
...
dialog.addEventListener("change", async (e) => {
  if(e.target.id === "genre" && e.target.value === "Sci-fi"){const f=e.target.form;open("scifi",{cards:SF.blank(),title:f.elements.title.value,scale:f.elements.scale.value});return;}
...
function writersRoom(m){
 if(m.narrativePack)return narrativeRoom(m);
 const draft=view.draft??[...E.storyOutline(m)];view.draft=draft;
 return modal('The writers’ room',`<h3>${h(m.title)}</h3><p>${h(m.premise)}</p><p>Your purchased draft is ready to film. Rewriting is optional and costs $50,000 per approved draft. Changes lock at greenlight; returning to the purchased draft before then is free, with no refund for past writing.</p>${[0,1,2].map(i=>`<section class="panel"><h3>Act ${i+1} · ${['Setup','Turning point','Resolution'][i]}</h3>${E.storyChoices(s,m,i,draft).map(c=>button(`${draft[i]===c.id?'✓ ':''}${h(c.title)}<small>${h(c.text)}${c.lock?' · 🔒 '+h(c.lock):''}</small>`,'storyPick',`data-act="${i}" data-choice="${c.id}" ${c.lock?'disabled':''} aria-pressed="${draft[i]===c.id}"`,'choice full')).join('')}</section>`).join('')}<section class="panel"><h3>Draft assessment</h3><p>${h(E.storyAssessment({...m,story:{acts:draft}}).notes)}</p><p>${draft[0]==='ensemble'?'Ensemble structure increases role difficulty by 4; your existing roles remain.':'Contained structure keeps the existing role difficulty.'} Locked choices expand your options; they do not guarantee a better movie.</p></section>${button('Approve rewrite · $50,000','saveStory',`data-id="${m.id}"`,'primary full')}${button('Restore purchased draft','restoreStory',`data-id="${m.id}"`,'outline full')}<p class="small">Changing an earlier act may require choosing a compatible later act before approval.</p>`,'THREE-ACT STORY');
}
function storyReport(m){
 if(m.scifiReception)return scifiReport(m);
 if(m.scifiCards)return "";
 if(m.narrativePack)return narrativeReport(m);
 if(!Number.isFinite(m.fans)||!Number.isFinite(m.critics))return '';
 const acts=E.storyOutline(m),assessment=E.storyAssessment(m);
 return `<section class="panel story-report"><span class="eyebrow">STORY REPORT CARD</span><h3>Your draft, on screen</h3>${acts.map((id,i)=>{const c=E.storyChoices(s,m,i,acts).find(x=>x.id===id);return `<p><strong>Act ${i+1}: ${h(c.title)}</strong><small class="block">${h(c.text)}</small></p>`;}).join('')}<div class="finance-lines"><div><span>Audience response</span><strong>${E.score(m.fans)}/100</strong></div><div><span>Critical response</span><strong>${E.score(m.critics)}/100</strong></div><div><span>Finished film quality</span><strong>${E.score(m.quality)}/100</strong></div></div><p>${h(assessment.notes)}</p><p class="small muted">These are whole-film results: casting, production, story choices and other events all contribute. ${m.story?.revisions??0} draft revision(s).</p></section>`;
}

const beatIcons=['✦','ϟ','↗','◎','◈','↪'];
function narrativeCast(m,draft,editable=false){
 return `<div class="story-cast">${N.packFor(draft).characters.map((role,i)=>{const option=role.options.find(o=>o.id===draft.characters[i]);const contract=m.contracts?.find(c=>c.role===i),actor=contract?E.person(s,contract.id):null;return `<article class="story-person"><div class="story-avatar">${actor?portrait(actor,80):`<span aria-hidden="true">${['✦','◆',N.packFor(draft).theme==='comedy'?'♫':N.packFor(draft).theme==='drama'?'✎':'⚙','◉'][i]}</span>`}</div><span class="eyebrow">${h(role.title)}</span><h4>${h(role.name)}</h4><strong>${h(option?.label??'Choose a role')}</strong><p>${h(option?.detail??'')}</p><small>${actor?'Played by '+h(actor.name):'Cast this part in packaging'}</small>${editable?`<details><summary>Change character</summary>${role.options.map(o=>button(h(o.label),'narrativeCast',`data-role="${i}" data-choice="${o.id}" aria-pressed="${o.id===draft.characters[i]}"`,'outline full')).join('')}</details>`:''}</article>`;}).join('')}</div>`;
}
function narrativeTimeline(draft,active=-2,interactive=false){
 const pack=N.packFor(draft),e=N.evaluate(draft);return `<ol class="story-timeline" aria-label="Your six story decisions">${pack.steps.map((name,i)=>{const invalid=e.errors.some(x=>x.step===i),c=e.selected[i];const inside=`<span class="beat-icon" aria-hidden="true">${beatIcons[i]}</span><small>${i<1?'ACT I':i<3?'ACT II':'ACT III'} · ${name}</small><strong>${h(c?.title??'Choose a scene')}</strong>${invalid?'<em>Needs revision</em>':''}`;return `<li class="${invalid?'broken':''} ${active===i?'current':''}">${interactive?button(inside,'narrativeStep',`data-step="${i}" ${active===i?'aria-current="step"':''}`,'story-stop'):inside}</li>`;}).join('')}</ol>`;
}
function narrativeRoom(m){
 const d=view.narrative??structuredClone(m.narrative);view.narrative=d;const step=view.step??-1,pack=N.packFor(d),e=N.evaluate(d),changed=JSON.stringify(d)!==JSON.stringify(m.narrative);
 const options=step>=0?N.choices(d,step):[];const shown=[...options.filter(c=>!c.lock),...options.filter(c=>c.lock&&(c.id===d.beats[step])).slice(0,1),...options.filter(c=>c.lock&&c.id!==d.beats[step]).slice(0,2)];
 const scene=step<0?narrativeCast(m,d,true):`<div class="scene-heading"><span class="scene-number" aria-hidden="true">${beatIcons[step]}</span><div><span class="eyebrow">${step+1} / 6 · ${step<1?'ACT I':step<3?'ACT II':'ACT III'}</span><h3>${pack.steps[step]}</h3><p>${h(pack.questions[step])}</p></div></div><div class="story-options">${shown.map(c=>button(`<span class="choice-mark" aria-hidden="true">${c.lock?'🔒':d.beats[step]===c.id?'✓':'○'}</span><strong>${h(c.title)}</strong><small>${h(c.text)}</small><span class="story-tag">${c.lock?'Needs '+h(c.lock):c.effects?pack.departmentLabel+' +'+E.accountMoney(c.effects):'Character / plot'}</span>`,'narrativePick',`data-choice="${c.id}" ${c.lock?'disabled':''} aria-pressed="${d.beats[step]===c.id}"`,'story-option')).join('')}</div>${options.filter(c=>c.lock).length>shown.filter(c=>c.lock).length?'<p class="small muted">More possibilities appear as earlier story facts change.</p>':''}`;
 return modal('Build your movie',`<div class="narrative-room ${step>=0?'editing-step':''}" data-theme="${pack.theme}"><header class="story-banner"><span class="eyebrow">${h(m.title)} · ${h(m.genre)}</span><h3>${h(pack.headline)}</h3><p>The purchased draft is complete. Film it as written, or change a part. Approving a changed draft costs $50,000.</p></header>${narrativeTimeline(d,step,true)}${button('♙ Characters','narrativeStep','data-step="-1"','outline')}<section class="story-workbench">${scene}</section>${e.errors.length?`<div class="story-repair" role="status"><strong>${e.errors.length} connection(s) need revision</strong><p>Your other selections stay in place. Resolve these before approval.</p>${e.errors.map(x=>`<p>${x.step>=0?pack.steps[x.step]+': ':''}${h(x.reason)}</p>`).join('')}</div>`:''}<div class="story-draft-summary"><span>${e.pressure} ${h(pack.departmentLabel.toLowerCase())} ambition</span><strong>${E.accountMoney(e.effects)} ${h(pack.departmentLabel.toLowerCase())} target</strong><small>Underfunding risks up to 8 quality points. Four contracted actors required; story deaths do not affect actors.</small></div><div class="story-navigation">${button(step<=0?'Characters':'← '+pack.steps[step-1],'narrativeStep',`data-step="${Math.max(-1,step-1)}"`,'outline')}${step<5?button('Next: '+pack.steps[step+1]+' →','narrativeStep',`data-step="${step+1}"`,'primary'):''}</div>${button(changed?'Approve this rewrite · $50,000':'Draft approved · ready to cast','saveNarrative',`data-id="${m.id}" ${!changed||!e.valid?'disabled':''}`,'primary full')}${button('Return to film packaging','movie',`data-id="${m.id}"`,'outline full')}${button('Restore purchased draft · free','restoreStory',`data-id="${m.id}" ${JSON.stringify(m.narrative)===JSON.stringify(m.originalNarrative)?'disabled':''}`,'text-button full')}<p class="small muted">Unapproved changes are discarded when you leave. All story edits lock at greenlight.</p></div>`,'THE WRITERS’ ROOM');
}
function narrativeReport(m){
 if(!Number.isFinite(m.fans)||!Number.isFinite(m.critics))return '';
 const pack=N.packFor(m.narrative),e=N.evaluate(m.narrative),links=e.links.slice(-3),loss=N.executionPenalty(m.narrative,m.budget);
 return `<section class="panel story-report narrative-report" data-theme="${pack.theme}"><span class="eyebrow">STORY REPORT CARD · ${m.narrativeRevisions??0} REVISIONS</span><h3>${h(m.title)}: the choices behind the film</h3>${narrativeTimeline(m.narrative)}<div class="payoff-grid"><article><span class="report-symbol">↗</span><h4>Setup → payoff</h4>${links.length?links.map(l=>`<div class="payoff-link"><strong>${h(e.selected[l.from].title)}</strong><span aria-hidden="true">↓</span><strong>${h(e.selected[l.to].title)}</strong><small>Connected by ${h(pack.labels[l.tag]??l.tag)}</small></div>`).join(''):'<p>The finale relies on character facts rather than a planted scene. Add a stronger earlier setup.</p>'}</article><article><span class="report-symbol">◈</span><h4>Connection to develop</h4><p>${h(e.weakness)}</p></article><article><span class="report-symbol">ϟ</span><h4>Production consequence</h4><p>${E.accountMoney(e.effects)} ${h(pack.departmentLabel.toLowerCase())} target; ${E.accountMoney(m.budget?.[pack.department]??0)} allocated.</p><strong>${loss?loss.toFixed(1)+' quality points lost to story ambition':'Story '+pack.departmentLabel.toLowerCase()+' target met'}</strong><p>Other production and casting factors still affect the finished film.</p></article></div><div class="scores">${stat('AUDIENCE',E.score(m.fans)+'/100')}${stat('CRITICS',E.score(m.critics)+'/100')}${stat('FILM QUALITY',E.score(m.quality)+'/100')}</div><p class="small muted">Structural observations explain selected story facts. These whole-film scores do not establish how viewers felt about any individual scene.</p><details><summary>The characters on screen</summary>${narrativeCast(m,m.narrative)}</details></section>`;
}
...
function scifiRoom(){
 const d=view.cards,valid=SF.validate(d),editing=!!view.id,cost=editing?50:100*(E.SCALES.indexOf(view.scale)+1);
 const changed=!editing||JSON.stringify(d)!==JSON.stringify(E.movie(s,view.id).scifiCards);
 return modal('Build your sci-fi movie',`<div class="sf-builder"><p class="sf-intro">Choose 1–2 cards in each section. Your first selection is primary; your second supports it. Tap a selected card to remove it.</p><form id="sf-form"><div class="form-grid"><label>Movie title<input name="title" required maxlength="60" value="${h(view.title)}" ${editing?'readonly':''} placeholder="Name your movie"></label><label>Production scope<select name="scale" ${editing?'disabled':''}>${E.SCALES.map(x=>`<option value="${x}" ${view.scale===x?'selected':''}>${E.scopeName(x)}</option>`).join('')}</select></label></div></form>${SF.SECTIONS.map((section,row)=>`<section class="sf-section"><header><div><span class="eyebrow">${section.name}</span><h3>${section.question}</h3></div><span>${d[section.id].length} / 2</span></header><div class="sf-cards">${section.cards.map((card,col)=>{const rank=d[section.id].indexOf(card);return button(`${sfArt(row,col)}<strong>${h(card)}</strong>${rank>=0?`<span class="sf-rank">${rank+1}</span>`:''}`,'sfPick',`data-section="${section.id}" data-card="${card}" aria-pressed="${rank>=0}" aria-label="${h(card)}${rank>=0?rank===0?', primary':', supporting':''}"`,'sf-card');}).join('')}</div></section>`).join('')}<section class="sf-focus"><span class="eyebrow">CREATIVE DEMANDS</span><p>${SF.selected(d).length?SF.demands(d).filter(([,n])=>n>0).slice(0,2).map(([k])=>h(SF.DIMENSIONS[k])).join(' and '):'Select ingredients to reveal your creative focus.'}</p><small>Cast, direction, writing and production determine execution. Additional cards are not automatic quality bonuses.</small></section><footer class="sf-footer"><div><strong>${E.accountMoney(cost)} ${editing?'rewrite':'screenplay'}</strong><small>${editing?'Locks when filming begins.':'Three weeks of development. Cast and production paid separately.'}</small></div><button type="submit" form="sf-form" class="primary" ${!valid||!changed?'disabled':''}>${editing?'Approve revised cards':'Commission screenplay →'}</button></footer></div>`,'SCI-FI CARD LAB');
}
...
function peopleBoard(){
 ...
 return `<section class="people-panel"><span class="eyebrow">PEOPLE & AUDIENCES</span><h3>Stories beyond the screen</h3>${fans.map(m=>`<div class="people-row"><div><strong>${h(m.title)}</strong><small>${h(m.fanCommunity.title)}</small></div>${button('Hear from fans','movie',`data-id="${m.id}"`,'outline')}</div>`).join('')}${promises.map(p=>`<div class="people-row"><div><strong>${h(p.name)}</strong><small>${p.personality.promise.movie?'Promised project underway':`${h(p.personality.promise.genre)} role · ${Math.max(0,p.personality.promise.due-s.week)} weeks to start filming`}</small></div>${button('Your promise','person',`data-person="${p.id}"`,'outline')}</div>`).join('')}</section>`;
}
function peopleFilm(m){
 ...
 if(f?.ready)content+=`<div class="fan-conversation"><span class="eyebrow">THE AUDIENCE HAS A VOICE</span><h3>${h(f.title)}</h3><blockquote>${h(f.text)}</blockquote>${f.choice?`<p>${h(f.response)}</p>`:`<div class="people-options">${button('Spotlight the fan conversation · $25,000','fanResponse',`data-id="${m.id}" data-choice="spotlight" ${m.stage!=='theaters'?'disabled':''}`,'outline')}${button('Thank fans and credit the cast · free','fanResponse',`data-id="${m.id}" data-choice="thanks"`,'outline')}${button('Listen without making promises · free','fanResponse',`data-id="${m.id}" data-choice="listen"`,'outline')}</div><p class="small muted">Spotlight: +8% theatrical demand for the next four weeks while still in theaters; no change to reviews and no guaranteed profit. Crediting the cast builds the lead’s trust. Respond once, or leave it for later.</p>`}</div>`;
...
function commissionDialog(){const c=s.marsTravel,d=c.deal;return modal('Mars Travel',`<div class="people-director"><div class="ceo-avatar">EV</div><div><span class="eyebrow">ELENA VALE · CEO</span><h3>“Make a future people can picture.”</h3></div></div><p>${h(c.last)}</p>${d?`<section class="people-panel"><h3>Your commission</h3><p>Release a sci-fi movie with <strong>Colony</strong> selected as a primary or supporting Setting card.</p><p><strong>${E.accountMoney(d.payment)}</strong> paid once on theatrical opening by ${E.date(d.due).label}, week ${E.date(d.due).week}. ${Math.max(0,d.due-s.week)} weeks remain.</p>${d.movie?`<p>Attached film: <strong>${h(E.movie(s,d.movie)?.title)}</strong></p>${button('Open the film','movie',`data-id="${d.movie}"`,'primary')}`:`<p>Choose a film before filming begins. Attachment is final; its release determines this deal.</p>${s.movies.filter(E.commissionEligible).map(m=>button('Attach '+h(m.title),'commissionBind',`data-id="${m.id}"`,'outline full')).join('')||'<p>No eligible sci-fi films yet.</p>'}${button('Build a sci-fi movie','scifiNew','','primary full')}`}<hr><p>One early extension: eight more weeks, 20% lower payment (${E.accountMoney(d.payment*.8)}).</p>${button(d.extended?'Extension already used':'Request the extension','commissionExtend',`${d.extended||s.week>d.due-4||d.due+8>=E.END?'disabled':''}`,'outline')}<p class="small muted">Request at least four weeks before the deadline. The extended deadline must fit within this studio run.</p></section>`:E.commissionAvailable(s)?`<section class="people-panel"><h3>The offer</h3><p>Release a <strong>Sci-Fi + Colony</strong> card film within <strong>40 weeks</strong> for <strong>${E.accountMoney(Math.min(1000,600+c.wins*200))}</strong>.</p><p>Accept, attach one film in development or packaging, then release it with Colony in its Setting cards. No minimum score, box office, or budget. No rights transfer or distribution exclusivity. Payment is studio income, not ticket sales.</p><p>A missed deadline, shelved film, or release without the required cards means no payment and a 52-week pause in requests. Two missed commissions end requests for this run. Deliveries unlock a larger offer after 12 weeks.</p>${button('Accept the commission','commissionAccept','','primary full')}${button('Decline for now · no penalty','commissionDecline','','outline full')}</section>`:`<p>${c.strikes>=2?'No more requests this run.':s.week+40>=E.END?'Not enough time remains for another 40-week commission.':`Elena may return in ${Math.max(0,c.nextOffer-s.week)} weeks.`}</p>`}<p class="small muted">Colony can be either Setting selection. Making the movie remains your responsibility; normal production and release costs apply.</p>${c.history.length?`<h3>Your history</h3>${c.history.slice(-5).reverse().map(x=>`<p><strong>${h(x.title)} · ${h(x.outcome)}</strong><small class="block">${h(x.reason)} ${x.payment?E.accountMoney(x.payment)+' paid.':''}</small></p>`).join('')}`:''}`,'AN INDUSTRY RELATIONSHIP');}
function commissionFilm(m){const d=s.marsTravel?.deal;if(d?.movie===m.id)return `<section class="people-panel"><span class="eyebrow">MARS TRAVEL COMMISSION</span><p>${E.accountMoney(d.payment)} for release within ${Math.max(0,d.due-s.week)} weeks. Colony must remain in your Setting cards.</p><p>${m.scifiCards?.setting.includes('Colony')?'✓ Required card is present.':'Colony is missing. Revise the cards before filming to qualify.'}</p>${button('Review deadline & extension','commission','','outline')}</section>`;if(d&&!d.movie&&E.commissionEligible(m))return `<section class="people-panel"><p>Mars Travel is waiting for a film. Attaching this project is final; release it with Colony by the agreed deadline.</p>${button('Attach this film to Mars Travel','commissionBind',`data-id="${m.id}"`,'outline')}</section>`;if(m.sponsorIncome)return `<p class="commission-paid">Mars Travel paid ${E.accountMoney(m.sponsorIncome)}. Included in studio receipts; excluded from box office.</p>`;return '';}
function delayPanel(m){const d=E.person(s,m.director.id),choices=E.delayChoices(s,m);return `<section class="decision-box"><div class="people-director">${portrait(d,72)}<div><span class="eyebrow">${h(d.name)} · DIRECTOR</span><h3>${h(m.event.title)}</h3></div></div><blockquote>${h(m.event.text)}</blockquote><p>Choose a creative response. Paying more cannot remove the time requirement.</p><div class="choice-stack">${choices.map(c=>button(`${h(c.label)}<small>${c.weeks} extra week(s) · ${c.damage?c.damage+' quality penalty':'quality protected'}${c.conflictWeeks?' · includes '+c.conflictWeeks+' weeks waiting for existing talent bookings':''}</small><small>${c.weeks?E.accountMoney(c.holdingPerWeek*c.weeks)+' total holding costs over the pause':'No pause holding costs'}. Expected wrap: ${E.date(c.wrap).label}, week ${E.date(c.wrap).week}.${c.wrap>=E.END?' Beyond the five-year window.':''}</small>`,'event',`data-id="${m.id}" data-choice="${c.choice}"`,'choice')).join('')}</div><p class="small muted">Paused productions retain crew at 10% of their normal weekly filming cost; studio overhead and loan payments continue. Filming progress stops, but other movies continue. Booking conflicts can extend the pause. Release is scheduled after wrap.</p>${s.marsTravel?.deal?.movie===m.id?`<p class="promise-note">Mars Travel deadline: ${Math.max(0,s.marsTravel.deal.due-s.week)} weeks away. You still need at least one week after wrap to release.</p>`:''}</section>`;}
function delayStatus(m){const d=m.delayPlan;if(!d)return '';if(m.stage==='filming'&&d.remaining)return `<section class="people-panel"><h3>Production paused · ${d.remaining} week(s) remaining</h3><p>Holding costs: ${E.accountMoney(m.weekly*.1)} per paused week. Filming resumes after the pause.</p></section>`;if(d.history.length)return `<details class="delay-history"><summary>Production setbacks · ${d.elapsed} extra calendar weeks so far</summary>${d.history.map(x=>`<p>${h(x.outcome)}</p>`).join('')}<p>Holding costs paid: ${E.accountMoney(d.holdingPaid)}. Included in film spending.</p></details>`;return '';}
...
function chemistryCandidate(m,p,role){
 ...
 return `<div class="chemistry-note"><strong>${h(CH.style(p))}</strong>${pairs.length?pairs.map(pair=>`<p>With ${h(E.person(s,pair.ids[1]).name)}: ${h(pair.label)} · ${chemistryEffect(pair.effect)} chemistry${pair.films?` · ${pair.films} shared films`:' · first pairing'}</p>`).join(''):'<p>Attach another actor to compare chemistry.</p>'}<small>Pair contributions are averaged per actor, up to ±4 performance points. Locks at filming.</small></div>`;
}
function chemistryFilm(m){
 const snapshot=m.castChemistry;
 if(!snapshot&&!['development','packaging'].includes(m.stage))return '';
 const pairs=snapshot?.pairs??CH.pairs(s,m);
 return `<section class="people-panel chemistry-panel"><span class="eyebrow">CAST CHEMISTRY</span><h3>${snapshot?'The people between the scenes':'Who brings out their best?'}</h3>${pairs.length?pairs.map(pair=>`<div class="chemistry-pair"><strong>${pair.ids.map(id=>h(E.person(s,id).name)).join(' & ')}</strong><p>${h(pair.label)} · ${chemistryEffect(pair.effect)} pair contribution · ${pair.films} previous shared films</p>${pair.outcome?`<p>${h(pair.outcome)}</p>`:''}</div>`).join(''):'<p>Attach at least two actors to discover how their working styles fit.</p>'}<p class="small muted">${snapshot?'Chemistry locked when filming started.':'Compare working styles while casting.'} Pair contributions are averaged for each actor, up to ±4 performance points. A pair averaging 60+ in delivered performances builds trust; below 60 builds tension. Shared history changes at wrap and carries into future films.</p></section>`;
}
