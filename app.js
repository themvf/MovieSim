import * as E from "./engine.js?v=0.12.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.12.0";
const BUILD = "0.10.0";
...
function openingDialog(m) {
  ...
  modal(h(m.title),`<div class="opening-report">
    <section data-opening-panel="results" ${section !== "results" ? "hidden" : ""}><div class="opening-hero"><span class="eyebrow">OPENING BOX OFFICE</span><strong class="opening-number">${E.money(m.opening)}</strong></div>${openingContext(m)}<p class="release-spending">Marketing spent: <strong>${E.accountMoney(m.marketingSpendAtRelease ?? m.campaignSpend)}</strong></p>${m.economyVersion === 2 && E.deliveryFactor(m)<.6 ? `<p class="notice-banner">The production struggled to deliver its promised scale. Advertising brought attention, but execution limited ticket demand.</p>` : ""}${expectationsReview(m)}<p class="muted small">This movie is still earning. Studio overhead is separate.</p></section>
...
function productionPreview(m) {
  ...
  for (const key of ["sets", "crew", "effects"]) {
    $(`${key}-cost`).textContent = E.accountMoney(b[key]);
    $(`${key}-description`).textContent =
      `${E.BUDGET_TIERS[plan[key]]} · ${E.BUDGET_DETAILS[key][plan[key]]} · ${E.productionNeeds(m, b).find((n) => n.key === key).status} (target ${E.accountMoney(E.productionNeeds(m, b).find((n) => n.key === key).target)})`;
  }
  ...
  $("production-preview").innerHTML =
    `<div class="budget-impact"><span class="eyebrow">WHAT THIS BUDGET BUYS</span><h3>Your production plan</h3><p>Each department is compared with this ${h(m.subgenre)} film’s needs. Extra spending in one department cannot fully cover a shortage in another. Strong writing and talent still matter.</p><small>Talent fees and marketing are separate. Spending less does not automatically cause production delays.</small></div><div class="schedule-explainer"><span class="eyebrow">${schedule.label.toUpperCase()} SHOOT</span><h3>${plan.duration} weeks: ${plan.duration < schedule.target ? "short of the recommended schedule" : plan.duration === schedule.target ? "meets this script’s needs" : "extra time for rehearsal and coverage"}</h3><p>${plan.duration < schedule.target ? "The compressed schedule reduces execution quality. More demanding productions need more time." : "Your team has time to execute the script. Extra weeks cost more and have diminishing benefits."}</p><strong>${plan.duration === 8 ? "10 weeks would cost " + E.accountMoney(E.productionCosts(s, m, b, 10).total - base.total) + " more." : `${costs.total >= base.total ? "+" : "−"}${E.accountMoney(Math.abs(costs.total - base.total))} versus an 8-week shoot.`}</strong><small>Recommended: ${schedule.target} weeks. Longer shoots have diminishing benefits and do not guarantee better reviews.</small></div><div class="finance-lines"><div><span>Cast, director & options · paid now</span><strong>${E.accountMoney(fee)}</strong></div><div><span>Production · paid over ${plan.duration} weeks</span><strong>${E.accountMoney(costs.total)}</strong></div><div><span>Production cost each week</span><strong>${E.accountMoney(costs.weekly)}</strong></div><div><span>Planned total including script</span><strong>${E.accountMoney(m.spent + fee + costs.total)}</strong></div><div><span>Cash after immediate fees</span><strong class="${s.cash - fee < 0 ? "negative" : ""}">${E.accountMoney(s.cash - fee)}</strong></div></div>${
      booked.length
