import {nextHire, standardOffer, deadlineWindows} from './experience.js?v=0.39.0';
import * as REC from "./reception.js?v=0.39.0";
import * as LIFE from "./studio-life.js?v=0.39.0";
import {GENRE_SYMBOLS} from "./card-genres.js?v=0.39.0";
import * as CL from "./clients.js?v=0.39.0";
import * as CH from "./chemistry.js?v=0.39.0";
import * as SF from "./scifi.js?v=0.39.0";
import * as N from "./narrative.js?v=0.39.0";
import * as E from "./engine.js?v=0.39.0";
import { portrait, poster, studioArt, settingCardArt, escapeHtml as h } from "./art.js?v=0.39.0";
const BUILD = "0.10.0";
...
function clientBoard(){
 return `<section class="client-board"><div class="section-title"><h2>People with a brief</h2></div><div class="client-grid">${CL.CLIENTS.map(c=>{const r=s.clients[c.id],d=r.deal;return `<button class="client-tile" data-action="client" data-client="${c.id}"><span class="client-avatar client-${c.id}" aria-hidden="true">${c.initials}</span><span><strong>${h(c.name)}</strong><small>${h(c.company)}</small><b>${d?`${Math.max(0,d.due-s.week)} weeks left`:CL.available(s,c.id)?'New brief':r.strikes>=2?'Requests ended':s.week+c.weeks>=E.END?'No new briefs':`Returns in ${Math.max(0,r.nextOffer-s.week)} weeks`}</b></span><span aria-hidden="true">→</span></button>`;}).join('')}</div></section>`;
}
