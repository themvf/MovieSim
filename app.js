...
import {GENRE_SYMBOLS} from "./card-genres.js?v=0.38.0";
import * as CL from "./clients.js?v=0.38.0";
import * as CH from "./chemistry.js?v=0.38.0";
import * as SF from "./scifi.js?v=0.38.0";
import * as N from "./narrative.js?v=0.38.0";
import * as E from "./engine.js?v=0.38.0";
import { portrait, poster, studioArt, escapeHtml as h } from "./art.js?v=0.38.0";
const BUILD = "0.10.0";
...
function slate() {
 ...
 ${lifeBanner()}${pressTeaser()}${clientBoard()}${commissionBoard()}${peopleBoard()}${opportunityBoard()}<div class="slate-layout"><section><div class="section-title"><h2>Your productions <span>${s.movies.length}</span></h2><div class="segmented">${[
   ["active", "Active"],
   ["streaming", "Streaming"],
   ["catalog", "Catalog"],
   ["all", "All"],
...
function handle(e) {
  ...
  switch (a) {
    ...
    case "buy":
      ...
      break;
    case "scifiNew": open("scifi",{genre:b.dataset.genre??"Sci-fi",cards:SF.blank(),title:"",scale:"Small"});break;
    ...
    case "endingPick": {view.endingCards??=[];const card=b.dataset.card,i=view.endingCards.indexOf(card);if(i>=0)view.endingCards.splice(i,1);else if(view.endingCards.length<2){const next=[...view.endingCards,card];if(!REC.valid(next)){toast('Those outcomes contradict each other. Remove one first.');break;}view.endingCards=next;}else{toast('Choose at most two endings. Remove one first.');break;}drawDialog();break;}
...
function cardArt(genre,row,col){return genre==='Sci-fi'?sfArt(row,col):`<span class="genre-card-art genre-${genre.toLowerCase()}" style="--card-variant:${col}" aria-hidden="true"><i>${GENRE_SYMBOLS[genre]?.[row]??'✦'}</i></span>`;}
