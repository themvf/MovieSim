...
function talents() {
  ...
  return `<div class="section-title"><h2>The people behind the pictures</h2><div class="segmented">${[
    ...
    .join(
      "",
    )}</div></div><div class="talent-grid">${list.map((p) => `<button class="talent-card" data-action="person" data-person="${p.id}">${portrait(p, 64)}<div><h3>${h(p.name)}</h3><p>${p.kind === "actor" ? "Actor" : "Director"} · ${h(p.gender)} · Age ${p.age}</p><div class="talent-tags">${p.retired ? pill("Retired") : talentBadge(p)}</div>${talentAccolades(p)}${E.freshFace(p) ? '<p class="fresh-note">No major credits yet. This could be their breakthrough.</p>' : ""}${genreStrengths(p)}${talentRatings(p)}<p class="fee-line">Expected fee <strong>${E.money(p.fee * 0.85)}–${E.money(p.fee * 1.12)}</strong></p></div></button>`).join("")}</div>`;
}
function studio() {
  const desc = {
...
function careerResults(m) {
  return `<section><h3>Cast & director: career changes</h3>${
    (m.careerChanges ?? [])
      .map((c) => {
        const p = E.person(s, c.id),
          before = Math.round(c.before),
          after = Math.round(c.after),
          delta = after - before;
        return `<div class="career-headline">${portrait(p, 40)}<div><strong>${h(p.name)}</strong><small class="block">${p.kind === "director" ? "Director" : h(m.roles[m.contracts.find((x) => x.id === p.id)?.role] ?? "Actor")}</small><p>Fame ${before} → ${after} <strong class="${delta > 0 ? "mint" : delta < 0 ? "negative" : "muted"}">${delta > 0 ? "↑ +" + delta : delta < 0 ? "↓ " + delta : "— Unchanged"}</strong></p><small>${delta > 0 ? "Their performance earned attention." : delta < 0 ? "Their performance hurt their reputation." : "This release did not change their fame."} Performance ${E.score(c.performance)}/100.</small></div></div>`;
      })
      .join("") || "<p>No career changes recorded for this earlier film.</p>"
  }</section>`;
}
function openingContext(m) {
  ...
  return `<div class="opening-context"><p><strong>${forecast}</strong><br>${E.boxOfficeStatus(m).label} compared with other ${E.scopeName(m.scale).toLowerCase()}s.</p><p>Studio receipts so far: <strong>${E.money(m.receipts)}</strong></p>${button(`Cast careers: ${gained} gained · ${lost} lost · ${changes.length - gained - lost} unchanged →`, "viewCareers", `data-id="${m.id}"`, "outline full")}</div>`;
}
...
function movieDetail(m) {
  ...
  return `<div class="detail-hero">${poster(m, true)}<div>${pill(labels[m.stage])}<h2>${h(m.title)}</h2><p>${h(m.genre)} / ${h(m.subgenre)} · ${E.scopeName(m.scale)}</p><p class="muted">${h(m.premise)}</p><p class="scope-description">${E.SCOPES[m.scale].description}</p>${m.release ? `<p class="small peach">Release locked: ${E.date(m.release).label}, week ${E.date(m.release).week}</p>` : ""}</div></div><div class="detail-stats">${stat("SPENT", E.money(m.spent))}${stat("STUDIO RECEIPTS", E.money(m.receipts))}${stat("SCRIPT ESTIMATE", ratingRange(E.range(m.scriptQuality, s.departments.Development)))}</div>
...
function criticsPanel(m) {
  return `<section class="panel"><span class="eyebrow">THE REVIEWS ARE IN</span><h3>Three critics. Three perspectives.</h3><p>Overall critics score: <strong>${E.score(m.critics)}/100</strong></p>${E.criticReviews(m).map(r=>`<article class="critic-review"><div class="section-title"><h3>${h(r.name)}</h3>${pill(`${E.score(r.score)}/100`)}</div><small class="muted">${h(r.taste)}</small><p>“${h(r.quote)}”</p></article>`).join("")}</section>`;
}
function streamingSummary(m) {
  ...
  return `<section class="panel"><h3>${deal ? h(deal.name) : "Streaming & licensing"}</h3>${deal ? `<p>${h(deal.label)} · ${Math.max(0,deal.endWeek-s.week)} weeks remaining</p><p>Paid upfront: <strong>${E.money(deal.upfront)}</strong></p>` : ""}<p>Earned to date: <strong>${E.money(m.catalog)}</strong></p><p>Next week: <strong>${E.money(E.catalogIncome({...s,week:s.week+1},m))}</strong></p><small class="muted">${deal && s.week < deal.endWeek ? "After 52 weeks, this agreement expires and automatic catalog licensing resumes. Sequel rights remain yours." : "Your catalog earns ongoing licensing income that declines over time."}</small></section>`;
}
function streamingDialog(m) {
  if (m.streamingDeal !== null || s.ended || s.epilogue) return modal("Streaming & licensing",streamingSummary(m),h(m.title));
  const offers=E.streamingOffers(m), selected=offers.find(o=>o.id===view.streamingChoice);
  return modal("Your movie’s next chapter",`<p><strong>${h(m.title)}</strong> has finished its theatrical run. Compare two 52-week offers.</p><div class="table-wrap"><table class="streaming-comparison"><thead><tr><th>Platform</th><th>Paid now</th><th>First week</th></tr></thead><tbody>${offers.map(o=>`<tr><td><strong>${h(o.name)}</strong><small class="block">${o.id === "exclusive" ? "Guaranteed payment" : "Payment + royalties"}</small>${button("Review offer", "reviewStreaming", `data-deal="${o.id}"`, "text-button")}</td><td>${E.money(o.upfront)}</td><td>${E.money(o.weekly)}</td></tr>`).join("")}</tbody></table></div>${selected ? `<section class="panel"><span class="eyebrow">${h(selected.name)}</span><h3>${h(selected.label)}</h3><p>${selected.id === "exclusive" ? `Receive ${E.money(selected.upfront)} today. No additional streaming income during this exclusive 52-week license.` : `Receive ${E.money(selected.upfront)} today. Weekly royalties begin next week at ${E.money(selected.weekly)}, then decline as the movie ages. More time earning can make this the better deal.`}</p>${button(`Sign with ${selected.name}`,"signStreaming",`data-id="${m.id}" data-deal="${selected.id}"`,"primary full")}</section>` : '<p class="muted">Review either offer to see its terms before signing.</p>'}<p class="muted small">After 52 weeks, automatic catalog licensing resumes. You keep sequel rights. No recoupment or talent ticket-revenue shares apply to these payments. Future royalties are not paid upfront when the demo ends.</p>${button("Decide later · no streaming income yet", "deferStreaming", `data-id="${m.id}"`, "outline full")}`,"STREAMING OFFERS");
}
function report(m) {
  const f = m.releaseFactors;
  return `${streamingSummary(m)}${successSummary(m)}${expectationsReview(m)}${criticsPanel(m)}${careerResults(m)}<section><div class="section-title"><h3>The release report</h3>${m.awards.length ? pill(`♜ ${m.awards.length} awards`, "gold-pill") : ""}</div><div class="scores">${stat("TEST SCREENING", m.screen == null ? "Not held" : E.score(m.screen))}${stat("CRITICS", E.score(m.critics))}${stat("FANS", E.score(m.fans))}</div><div class="box-chart" role="img" aria-label="Weekly box office: ${m.boxWeeks.map((v, i) => `week ${i + 1} ${E.money(v)}`).join(", ")}">${m.boxWeeks.map((v, i) => `<div><i style="height:${Math.max(3, (v / Math.max(...m.boxWeeks)) * 110)}px"></i><small>W${i + 1}</small></div>`).join("")}</div><div class="finance-lines"><div><span>Gross box office</span><strong>${E.money(m.gross)}</strong></div><div><span>Your share of ticket sales</span><strong>${Math.round(m.share * 100)}%</strong></div><div><span>Release costs / advance recovered by distributor</span><strong>${E.money(m.recouped ?? 0)}</strong></div><div><span>Still to recover before ticket payments</span><strong>${E.money(m.recoupRemaining ?? 0)}</strong></div><div><span>Upfront payment received</span><strong>${E.money(m.advance)}</strong></div><div><span>Streaming & licensing</span><strong>${E.money(m.catalog)}</strong></div><div><span>Total studio receipts</span><strong>${E.money(m.receipts)}</strong></div><div><span>Talent revenue shares paid</span><strong>${E.money(m.participationPaid ?? 0)}</strong></div><div><span>Total movie costs (including talent shares)</span><strong>${E.money(m.spent)}</strong></div><div><span>Movie profit / loss to date</span><strong class="${m.receipts >= m.spent ? "mint" : "negative"}">${E.money(m.receipts - m.spent)}</strong></div></div><div class="analysis-note"><p>${E.productionNeeds(
    m,
  )
    .map(
      (n) =>
        `${{ sets: "Sets", crew: "Crew", effects: "Effects" }[n.key]}: ${n.status.toLowerCase()}`,
    )
    .join(
      " · ",
    )}. Schedule: ${m.duration} weeks versus ${E.recommendedWeeks(m)} recommended.</p><span class="eyebrow">WHAT WE LEARNED / RESEARCH LEVEL ${s.departments.Research}</span><p>${m.craft < 45 ? "Production values fell short of the project’s ambition." : m.craft > 75 ? "Production spending translated into strong craft." : "Production values were serviceable, with room to improve."} ${m.fans > 75 ? "Audiences are giving the movie strong word of mouth." : m.fans < 50 ? "Weak audience response is limiting repeat business." : "Audience response is mixed to positive."} ${f.reach < 20 ? "Limited marketing held back opening awareness." : "Your campaign put the film in front of an audience."} ${f.rival > 0.3 ? "A crowded release window divided attention." : "Competition was manageable."} ${f.season > 1 ? "The seasonal audience boost helped." : ""}</p>${s.departments.Research >= 2 ? `<p>Average performance ${Math.round(m.performances.reduce((a, v) => a + v, 0) / m.performances.length)}/100 · Craft ${Math.round(m.craft)}/100. ${m.penalty ? `Production compromises reduced quality by approximately ${Math.round(m.penalty)} points.` : "No unresolved production compromises."}</p>` : ""}${s.departments.Research >= 3 ? `<p>Estimated competition reduction: ${Math.round((1 - 1 / (1 + f.rival)) * 100)}%. Seasonal audience boost: ${Math.round((f.season - 1) * 100)}%. Awareness index: ${Math.round(f.reach)}. The opening also includes unpredictable audience demand.</p>` : ""}</div></section>`;
}
function drawDialog() {
  if (!view) return;
  const v = view,
  ...
  if (v.kind === "script") {
    ...
    return modal(
      h(sc.title),
      `<div class="detail-hero">${poster(sc, true)}<div>${pill(E.scopeName(sc.scale))}<h3>${h(sc.genre)} / ${h(sc.subgenre)}</h3><p>${h(sc.premise)}</p><p>Script quality estimate: <strong>${ratingRange(E.range(sc.quality, s.departments.Development))}</strong></p><p>Role difficulty: <strong>${sc.difficulty}/100</strong></p><p>Cast: ${sc.roles.join(", ")}</p></div></div><p class="scope-description">${E.SCOPES[sc.scale].description}</p><p>Includes the screenplay and sequel rights. You can rename the movie after acquisition. Production, talent, and marketing are separate expenses.</p>${button(`Acquire screenplay · ${E.money(sc.price)}`, "buy", `data-script="${sc.id}"`, "primary full")}`,
      "SCRIPT ACQUISITION",
    );
  }
  if (v.kind === "original")
  ...
  if (
    ...
    return modal(
      ...
      `<p>Select your marketing plan, including $0 if you want no campaign. Nothing is purchased until you confirm.</p>${marketingChoices(m)}<p><strong>Budget: ${E.money(m.campaignSpend + (v.selectedCampaigns ?? []).reduce((sum,i) => sum+E.CAMPAIGNS[i].cost,0))}</strong></p>${button("Confirm marketing budget →", "confirmMarketing", `data-id="${m.id}" ${!v.marketingChoice && !m.campaignSpend ? "disabled" : ""}`, "primary full")}`,
      h(m.title),
    );
  }
  ...
  if (v.kind === "opening")
    return modal(
      "The numbers are in.",
      `<div class="opening-reveal"><span class="eyebrow gold">OPENING WEEKEND</span><h2>${h(m.title)}</h2><strong class="opening-number">${E.money(m.opening)}</strong><p>GROSS BOX OFFICE</p>${openingContext(m)}${criticsPanel(m)}${expectationsReview(m)}${releaseRecap(m)}${button("Consider a sequel", "openingSequel", `data-id="${m.id}"`, "outline full")}<details><summary>Forecast & detailed performance report</summary>${poster(m, true)}<div class="scores">${stat("CRITICS", E.score(m.critics))}${stat("FANS", E.score(m.fans))}${stat("YOUR FIRST RECEIPTS", E.money(Math.max(0, m.receipts - m.advance)))}</div><p class="muted">${m.fans > 75 ? "The audience is talking. Now let’s see how long the run lasts." : m.fans < 50 ? "A tough first audience. Keep a close eye on further marketing spending." : "The first weekend is only the beginning. Word of mouth takes over from here."}</p></details>${button("Back to the lot →", "dismissNotice", "", "primary full")}</div>`,
      "THE BOX OFFICE",
    );
...
function productionPreview(m) {
  ...
  for (const key of ["sets", "crew", "effects"]) {
    ...
    $(`${key}-description`).textContent =
      `${E.BUDGET_TIERS[plan[key]]} · ${E.BUDGET_DETAILS[key][plan[key]]} · ${E.productionNeeds(m, b).find((n) => n.key === key).status} (target ${E.money(E.productionNeeds(m, b).find((n) => n.key === key).target)})`;
  }
  ...
  if ($("production-total"))
    $("production-total").innerHTML =
      `<strong>Plan total ${E.money(m.spent + fee + costs.total)}</strong><small>Production still to pay: ${E.money(costs.total + s.movies.filter((x) => x.id !== m.id).reduce((v, x) => v + E.remaining(x), 0))} across your slate</small><strong class="${s.cash - fee - costs.total - s.movies.filter((x) => x.id !== m.id).reduce((v, x) => v + E.remaining(x), 0) < 0 ? "negative" : ""}">Cash after commitments: ${E.money(s.cash - fee - costs.total - s.movies.filter((x) => x.id !== m.id).reduce((v, x) => v + E.remaining(x), 0))}</strong><small>Before overhead, loan payments and release costs.</small>`;
  const booked = [
  ...
  $("production-preview").innerHTML =
    `<div class="budget-impact"><span class="eyebrow">WHAT THIS BUDGET BUYS</span><h3>Your production plan</h3><p>Each department is compared with this ${h(m.subgenre)} film’s needs. Extra spending in one department cannot fully cover a shortage in another. Strong writing and talent still matter.</p><small>Talent fees and marketing are separate. Spending less does not automatically cause production delays.</small></div><div class="schedule-explainer"><span class="eyebrow">${schedule.label.toUpperCase()} SHOOT</span><h3>${plan.duration} weeks: ${plan.duration < schedule.target ? "short of the recommended schedule" : plan.duration === schedule.target ? "meets this script’s needs" : "extra time for rehearsal and coverage"}</h3><p>${plan.duration < schedule.target ? "The compressed schedule reduces execution quality. More demanding productions need more time." : "Your team has time to execute the script. Extra weeks cost more and have diminishing benefits."}</p><strong>${plan.duration === 8 ? "10 weeks would cost " + E.money(E.productionCosts(s, m, b, 10).total - base.total) + " more." : `${costs.total >= base.total ? "+" : "−"}${E.money(Math.abs(costs.total - base.total))} versus an 8-week shoot.`}</strong><small>Recommended: ${schedule.target} weeks. Longer shoots have diminishing benefits and do not guarantee better reviews.</small></div><div class="finance-lines"><div><span>Cast, director & options · paid now</span><strong>${E.money(fee)}</strong></div><div><span>Production · paid over ${plan.duration} weeks</span><strong>${E.money(costs.total)}</strong></div><div><span>Production cost each week</span><strong>${E.money(costs.weekly)}</strong></div><div><span>Planned total including script</span><strong>${E.money(m.spent + fee + costs.total)}</strong></div><div><span>Cash after immediate fees</span><strong class="${s.cash - fee < 0 ? "negative" : ""}">${E.money(s.cash - fee)}</strong></div></div>${
      booked.length
