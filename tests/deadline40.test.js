import {ownedGame} from './helpers/owned-collection.js';
...
test('rush requires a delivered commission, not merely an accepted or missed one',()=>{
 const s=ownedGame(1);s.week=8;assert.equal(CL.available(s,'midnight'),false);
...
test('planning assumptions expose buffer and do not move accepted deadlines',()=>{
 const s=ownedGame(2);CL.accept(s,'starlight');const d=s.clients.starlight.deal;
...
test('flexible extension is costly, once only, and still available two weeks before deadline',()=>{
 const s=ownedGame(3);C.accept(s);assert.equal(s.marsTravel.deal.due,22);s.week=20;C.extend(s);
 ...
 const t=ownedGame(3);C.accept(t);t.week=21;assert.throws(()=>C.extend(t));
});
