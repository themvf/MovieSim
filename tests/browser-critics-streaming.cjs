...
(async()=>{
 ...
 await page.locator('.comparison-row').nth(2).scrollIntoViewIfNeeded();await page.screenshot({path:'test-results/comparison-rows-mobile.png'});
 ...
 await click('[data-action="filter"][data-filter="streaming"]');await click('[data-action="movie"]');await click('[data-action="streaming"]');
 ...
 await click('[data-action="filter"][data-filter="active"]');assert.equal(await page.locator('.movie-card').count(),0);await click('[data-action="filter"][data-filter="streaming"]');assert.equal(await page.locator('.movie-card').count(),1);
