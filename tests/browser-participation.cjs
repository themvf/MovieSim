...
(async () => {
  ...
  await p.evaluate(async () => {
    const E = await import("./engine.js?v=0.7.0");
    const s = E.newGame(78);
    const m = E.act(s, "buy", { script: s.market[0].id });
    m.scale = "Blockbuster";
    m.difficulty = 80;
    const star = s.people.find((p) => p.kind === "actor");
    star.star = 80;
    star.majorCredits = 8;
    star.genres[m.genre] = 80;
  ...
  const id = await p.evaluate(
    () => JSON.parse(localStorage.getItem("moviesim-save-v1")).people[0].id,
  );
  ...
  if (s.movies[0].contracts[0].grossShare !== 0.05)
    throw Error("Demand not signed");
  await b.close();
  console.log(
    "PASS: star demand visible before signing and persists at 5%; mobile offer",
  );
})().catch((e) => {
  console.error(e);
  process.exit(1);
