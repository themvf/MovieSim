import {GENRE_CARDS} from "./card-genres.js?v=0.46.0";
...
export const genres=['Sci-fi','Horror','Comedy'];
export function sections(genre='Sci-fi'){return genre==='Sci-fi'?SECTIONS:(GENRE_CARDS[genre]??[]).map(([id,name,question,cards])=>({id,name,question,cards}));}
...
export function validate(d,genre="Sci-fi"){return !!d && genres.includes(genre) && sections(genre).every(s=>Array.isArray(d[s.id])&&d[s.id].length>=1&&d[s.id].length<=2&&new Set(d[s.id]).size===d[s.id].length&&d[s.id].every(c=>s.cards.includes(c)))&&Object.keys(d).every(k=>sections(genre).some(s=>s.id===k));}
export function selected(d,genre="Sci-fi"){return sections(genre).flatMap((s,r)=>(d?.[s.id]??[]).map((name,rank)=>({name,rank,row:r,col:s.cards.indexOf(name),dimension:dimensions[r*5+s.cards.indexOf(name)]})));}
export function demands(d,genre="Sci-fi"){const w={story:0,acting:0,direction:0,craft:0};for(const c of selected(d,genre))w[c.dimension]+=c.rank?0.5:1;return Object.entries(w).sort((a,b)=>b[1]-a[1]);}
export function premise(d,genre="Sci-fi"){
 if(genre!=="Sci-fi")return `${d.characters.join(" and ")} pursue ${d.objective.join(" and ").toLowerCase()} in a ${d.setting.join(" and ").toLowerCase()}. ${d.concept.join(" and ")} meets ${d.complication.join(" and ").toLowerCase()}.`;
...
export function evaluate(m,fanNoise=0,criticNoise=0){
 if(!validate(m.scifiCards,m.genre??"Sci-fi"))throw Error('Choose 1–2 different cards in every section.');
 ...
 const demand=Object.fromEntries(demands(m.scifiCards,m.genre??"Sci-fi")),total=Object.values(demand).reduce((a,b)=>a+b,0);
 ...
 const audienceNames=m.genre==='Horror'?['Creature-feature fans','Suspense fans','Psychological horror fans','Character-focused viewers','Casual moviegoers']:m.genre==='Comedy'?['Visual comedy fans','Comic timing fans','Satire fans','Ensemble comedy fans','Casual moviegoers']:weights.map(g=>g.name);
 const groups=weights.map((g,i)=>{const fit=Object.keys(values).reduce((n,k)=>n+g.w[k]*demand[k]/total,0);return {name:audienceNames[i],weight:0.08+fit,score:Math.round(clamp(Object.keys(values).reduce((n,k)=>n+values[k]*g.w[k],0)+adjustment+fanNoise+(m.audienceBias??0)))};});
