import { scopeName } from "./engine.js?v=0.22.0";
...
export function portrait(p, size = 56) {
  if(Number.isInteger(p.photoActor)&&p.photoActor>=1&&p.photoActor<=34)return `<img class="portrait actor-headshot" src="./assets/actors/headshot-${p.photoActor}.jpg" width="${size}" height="${size}" alt="${escapeHtml(p.name)} headshot" loading="lazy">`;
  const n = p.look || 0,
