import { packFor } from "./narrative.js?v=0.43.0";
import { scopeName } from "./engine.js?v=0.43.0";
...
export function poster(m, large = false) {
  if(m.scifiCards&&m.genre!=='Sci-fi')return `<div class="poster ${large?'large':''} genre-poster genre-${m.genre.toLowerCase()}" role="img" aria-label="${escapeHtml(m.title)} movie poster"><span class="poster-top">${escapeHtml(m.genre)}</span><span class="genre-poster-symbol" aria-hidden="true">${m.genre==='Horror'?'◒':'✦'}</span><strong>${escapeHtml(m.title)}</strong><span class="poster-bottom">${escapeHtml(m.scifiCards.tone.join(' · '))}</span></div>`;
