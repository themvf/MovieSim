...
import * as PRESS from "./press.js?v=0.42.0";
import * as CH from "./chemistry.js?v=0.42.0";
import * as C from "./commissions.js?v=0.42.0";
import * as D from "./delays.js?v=0.42.0";
...
import * as P from "./personality.js?v=0.42.0";
export const ensurePersonalities=P.ensure;
export const workingStyle=P.style;
import * as SF from "./scifi.js?v=0.42.0";
import { authoredSpecs, evaluate as evaluateNarrative, executionPenalty } from "./narrative.js?v=0.42.0";
...
for (const genre of COL.DECKS.genre) { GENRES[genre] ??= []; if (!GENRES[genre].includes("Original")) GENRES[genre].push("Original"); }
export const SCOPES = {
  Small: {
...
function validMovieFinances(m) {
  if(m.endingCards && (!m.scifiCards || !REC.valid(m.endingCards) || !REC.PROMISES.includes(m.publicPromise??"As written"))) return false;
...
export function migrateSave(s) {
  ...
  for (const m of s.movies) {
    if(m.scifiCards&&SF.validate(m.scifiCards,m.genre)){
      const identity=SF.identity(m.scifiCards,m.genre);
      m.premise=identity.premise;m.roleDescriptions=identity.roleDescriptions;
      // Only pre-production projects adopt corrected mechanical subtype; finished economics stay fixed.
      if(['development','packaging'].includes(m.stage))m.subgenre=identity.subgenre;
    }
...
export function saveForecast(s, m) {
  ...
  m.responseExpectations={audience:m.screeningModel?.version===2?{low:m.screeningModel.low,high:m.screeningModel.high}:band(m.screen ?? center),critics:band(center),week:s.week};
  m.expectations = {
    low,
    high,
    week: s.week,
...
function script(s) {
  const genre = pick(s, Object.keys(GENRES).filter(g=>GENRES[g].length>1)),
  ...
  const subgenre = pick(s, GENRES[genre].filter(g=>g!=="Original"));
...
export const remaining = (m) =>
  m.stage === "filming" ? m.weekly * Math.max(0, m.duration - m.progress) + (m.delayPlan?.remaining??0)*m.weekly*.1 : 0;
export const burn = (s) =>
  overhead(s) +
  loanPayment(s) +
  s.movies
    .filter((m) => m.stage === "filming")
    .reduce((a, m) => a + D.pauseCost(m), 0);
...
function addMovie(s, sc, parent = null, developing = false) {
  ...
  if(m.movieCards){Object.assign(m,COL.identity(m.movieCards));delete m.cardReception;}
  else if(m.scifiCards) Object.assign(m,SF.identity(m.scifiCards,m.genre));
...
export function act(s, type, a = {}) {
  ...
  switch (type) {
    ...
    case "claimPack": return COL.claim(s,a.family);
    case "cardMovie": {
      COL.ensure(s);
      if (!COL.valid(a.cards,s)) throw Error("Fill the story slots with cards you own.");
      if (!SCALES.includes(a.scale)) throw Error("Choose a production scope.");
      const title=String(a.title??'').trim().slice(0,60);
      if (!title) throw Error("Give your movie a title.");
      const cost=100*(SCALES.indexOf(a.scale)+1)*LIFE.scriptMultiplier(s,a.cards.genre);
      if(s.cash<cost)throw Error("Not enough cash for this screenplay.");
      const sc={id:`s${s.next++}`,title,...COL.identity(a.cards),movieCards:structuredClone(a.cards),scale:a.scale,price:cost,quality:clamp(roll(s,42,85)+s.departments.Development*2,20,96),difficulty:60,art:roll(s,0,9999)};
      if(a.pitch)LIFE.checkPitch(s,a.pitch,a.cards.genre,a.cards);
      m=addMovie(s,sc,null,true);if(a.pitch)LIFE.attachPitch(s,m,a.pitch); break;
    }
    case "cardRewrite": {
      stage(m,["development","packaging"]);COL.ensure(s);
      if(!COL.valid(a.cards,s))throw Error("Fill the story slots with cards you own.");
      if(!['continuation','prequel'].includes(a.mode??'continuation'))throw Error("Choose sequel or prequel.");
    ...
    case "scifiRewrite": {
      ...
      const endings=a.endingCards??m.endingCards,promise=a.publicPromise??m.publicPromise??'As written';
      if(endings&&!REC.valid(endings)||!REC.PROMISES.includes(promise))throw Error('Choose 1–2 compatible ending cards and a campaign promise.');
      if(JSON.stringify(m.scifiCards)===JSON.stringify(a.cards)&&JSON.stringify(endings)===JSON.stringify(m.endingCards)&&promise===(m.publicPromise??'As written')) return m;
      if(s.cash<50)throw Error("You need $50,000 to approve revised cards.");
      debit(s,50);m.spent+=50;delete m.narrativePack;delete m.narrative;delete m.originalNarrative;m.scifiCards=structuredClone(a.cards);if(endings)m.endingCards=structuredClone(endings);m.publicPromise=promise;Object.assign(m,SF.identity(a.cards,m.genre));return m;
    }
    ...
    case "original": {
      ...
      if(a.endingCards&&(!a.scifiCards||!REC.valid(a.endingCards))||a.publicPromise&&!REC.PROMISES.includes(a.publicPromise))throw Error('Choose 1–2 compatible ending cards and a campaign promise.');
      ...
      if(a.endingCards){sc.endingCards=structuredClone(a.endingCards);sc.publicPromise=a.publicPromise??"As written";}
      if(a.scifiCards){ sc.scifiCards=structuredClone(a.scifiCards);Object.assign(sc,SF.identity(a.scifiCards,a.genre)); }
    ...
    case "sequel": {
      ...
      if(m.movieCards){sc.movieCards=COL.sequel(m.movieCards);Object.assign(sc,COL.identity(sc.movieCards));sc.sequelMode="continuation";}
    ...
    case "greenlight": {
      ...
      if(m.movieCards){if(!COL.valid(m.movieCards,s))throw Error("Choose owned cards before filming.");const issues=COL.continuity(s.movies.find(x=>x.id===m.parent)?.movieCards,m.movieCards,m.sequelMode);if(issues.length)throw Error(issues.join(" "));}
    ...
    case "event": {
      ...
      if(m.event.kind==="delay"){let choice=a.choice==="pay"?"wait":a.choice==="split"?"rework":a.choice==="cut"?"coverage":a.choice;if(m.event.index===5&&choice==="coverage")choice="rework";D.plan(s,m,choice);if(m.event.index===5)LIFE.crewChoice(s,m,choice);log(s,D.resolve(s,m,choice),"action");break;}
    ...
    case "screen": {
      ...
      debit(s, 45, m);
      if(m.scifiReception?.version===2){
        const radius=Math.max(3,11-s.departments.Research*2);
        m.screen=clamp(Math.round(m.fans+roll(s,-radius,radius)),0,100);
        m.screeningModel={version:2,radius,low:Math.max(0,m.screen-radius),high:Math.min(100,m.screen+radius)};
      }else {
        m.screen = clamp(Math.round(m.quality + roll(s, -25, 25) * (1 - s.departments.Research * 0.08)));
      }
      break;
    }
    case "campaign": {
...
function finish(s, m) {
  ...
  if(m.movieCards){m.cardReception=COL.evaluate(m,m.fans-m.quality-(m.audienceBias??0),m.critics-m.quality-(m.criticBias??0));m.fans=m.cardReception.fans;m.critics=m.cardReception.critics;}
  else if(m.scifiCards){
    const delivered={...m,scriptQuality:m.storyExecution??m.scriptQuality},fn=m.fans-m.quality-(m.audienceBias??0),cn=m.critics-m.quality-(m.criticBias??0);
...
function opening(s, m) {
  ...
  LIFE.release(s,m);
  s.prestige = clamp(s.prestige + Math.max(0, m.critics - 60) / 9);COL.ensure(s);
  log(
    s,
...
export function revealAward(s, year) {
  ...
  if (r.ours) {
    ...
    s.prestige = clamp(s.prestige + 8);COL.ensure(s);
    if (r.person) {
...
function nextWeek(s) {
  ...
  for (const m of s.movies) {
    ...
    if (m.stage === "filming" && m.delayPlan?.remaining>0) {
      const holding=D.pauseCost(m);debit(s,holding,m);m.delayPlan.holdingPaid+=holding;m.delayPlan.remaining--;m.delayPlan.elapsed++;
    } else if (m.stage === "filming") {
      debit(s, m.weekly, m);
      m.progress++;
    ...
    if (m.stage === "theaters") {
      const age = s.week - m.theaterStart,
        hold = m.scifiReception?.version===2?REC.retention(m.fans,m.critics):0.35 + m.fans / 200;
      let gross =
        age === 0
          ? m.opening
